...
public final class DamageItemCommands {
    ...
    private static int giveCritKit(
        ...
        give(player, TestItemFactory.critDamageSword());
    ...
    private static int giveAffixKit(
        ...
        give(player, TestItemFactory.affixHighestLevelProbe());
    ...
    private static int giveAffixHighestLevelProbe(CommandSourceStack source) {
        return CommandFeedback.requirePlayer(source)
                .map(player -> giveAffixHighestLevelProbe(source, player))
                .orElse(0);
    }

    private static int giveAffixHighestLevelProbe(
            CommandSourceStack source,
            ServerPlayer player
    ) {
        give(player, TestItemFactory.affixHighestLevelProbe());
