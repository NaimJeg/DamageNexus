...
final class DamageNexusSecurityGameTests {
    ...
    private static void verifyCommandPermissions() {
        ...
        for (String command : List.of(
                ...
                "damagenexus mob baseline at 120.5 65 200.5",
                "damagenexus mob baseline at ~ ~1 ~ immortal",
                "damagenexus mob baseline at ^ ^1 ^ immortal",
                "damagenexus mob zombie at ~ ~ ~ immortal",
                "damagenexus mob cow at ~ ~ ~ immortal",
                "damagenexus mob spider at ~ ~ ~ immortal",
                "damagenexus mob iron at ~ ~ ~ immortal",
                "damagenexus mob diamond at ~ ~ ~ immortal",
            ...
            var parse = dispatcher.parse(command, administrator);
            if (parse.getReader().canRead()
                    || !parse.getExceptions().isEmpty()) {
                throw new AssertionError(
                        "Required command syntax did not parse: " + command
                );
            }
        }
